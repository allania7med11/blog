...
function useSection({ current, action, dispatch }: UseSection) {
  ...
  const title =
    action === "create"
      ? `Create new ${current.type}`
      : `Update ${current.type}`;
  const methods = {
    updateSection: ({ target }: UpdateInput) =>
      updateState({ [target.name]: target.value }),
    updateBody: (html: string) => updateState({ body: html }),
...
function useModal(dispatch: Dispatch) {
  ...
  return [modalRef, updateShow, containerRef] as const;
}
const actionsInf = {
  create: {
    btn: "btn bg-primary",
...
const Form: FC<{ margin: string }> = ({ margin }) => {
  ...
  const [section, title, { updateSection, updateBody, submit }] = useSection({
    current,
    action,
    dispatch,
  });
  const [modalRef, updateShow, containerRef] = useModal(dispatch);
  const actionInf = actionsInf[action];
  return (
    <div ref={modalRef} onClick={updateShow} className={modal.modal}>
      <div ref={containerRef} className={margin}>
        <div className={`${card.card} mt-16 max-w-4xl z-10`}>
